...
public class IocMainTest {
	...
	public void testRangerDao(){
		...
		//DL 을 통해 스프링 커테이너에 스프링 빈을 요청
		//IRangerDao rangerDao = context.getBean("rangerDao");
		
		//변경 방법
		//스프링 컨테이너 생성을 junit runner에게 위입
	...
	/**
	 * 같은 이름의 스프링 빈(scope = singleton)을
	 *  두번 주입 받았을 떄 해당 객체가 동일한 객체인지 비교 
	 */
	@Test
	...
	/**
	 * 같은 클래스로 선언된 서로 다른 스프링 빈(singleton)이
	 * 디자인 패턴의 정의대로 동일한지 두 스프링 빈이 동일한지 테스트  
	 */
	@Test
	public void testSpringSingletonBean2() {
		//디자인 패턴에 의하면 같은 클래스로부터 하나의 인스턴스만 존재하게 하는 패턴이
		//싱글톤
		//rangerDaoSpringBean(rangerDao)과 rangerDao(rangerDao3) 스프링 빈은 서로 같은 RangerDaoImpl
		//클래스로부터 생성된 객체.
		//디자인 패턴의 정의에 의해서 두개의 객체는 서로 같아야 된다.
		//assertEquals(rangerDao, rangerDao3);
		
		//스프링 bean scope에서 이야기하는 singleton은 스프링 이름 단위로 객체가 생성된다.
		//rangerDaoSpringBean과 rangerDao는 같은 클래스로 부터 생성된었지만
